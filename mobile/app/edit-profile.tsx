...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, TextInput, KeyboardAvoidingView, Platform, ScrollView, Modal, Pressable, Alert, ActivityIndicator } from 'react-native';
...
export default function EditProfileScreen() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setYearLevel(profile.yearLevel || '1st Year');
      setSection(profile.section || '');
      setAge(profile.age ? profile.age.toString() : '');
      
      let mappedSex = 'Male';
      if (profile.sex === 'FEMALE') mappedSex = 'Female';
      if (profile.sex === 'PREFER_NOT_TO_SAY' || profile.sex === 'OTHER') mappedSex = 'Prefer not';
      setSex(mappedSex);

      if (profile.healthConcerns && profile.healthConcerns.length > 0) {
        setSelectedConcerns(profile.healthConcerns);
      }
    } catch (error) {
